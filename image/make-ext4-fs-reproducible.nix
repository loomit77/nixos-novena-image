# Builds a reproducible ext4 image containing a populated /nix/store with
# the closure of store paths passed in the storePaths parameter, in addition
# to the contents of a directory that can be populated with commands.
#
# Based on nixos/lib/make-ext4-fs.nix from the nixpkgs revision pinned by
# this project. The Novena-specific variant adds a fixed directory hash seed
# to mkfs.ext4 so repeated realizations are byte-reproducible.
{
  pkgs,
  lib,
  ...
  # mke2fs otherwise generates a random directory hash seed.
  # Use a fixed UUID-shaped value for reproducible filesystem images.
  hashSeed ? uuid,
  e2fsprogs,
  libfaketime,
  perl,
...
pkgs.stdenv.mkDerivation {
  ...
  buildCommand = ''
    ...
    faketime -f "1970-01-01 00:00:01" fakeroot mkfs.ext4 \
      -L ${volumeLabel} \
      -U ${uuid} \
      -E hash_seed=${hashSeed} \
      -d ./rootImage \
      $img

    export EXT2FS_NO_MTAB_OK=yes
    # I have ended up with corrupted images sometimes, I suspect that happens
    # when the build machine's disk gets full during the build.
